...
<g:if test="${measurement.type == MeasurementTypeName.BLOODSUGAR.name()}">
    <script type="text/javascript">
        ...
        var prepareMeasurement = function (series, index) {
            var originalDate = new Date(series[index][0]);
            var date = resetMeasurementDate(originalDate);
            series[index].push(formatDate(originalDate)); //We need to hold on to the original date so we can show it in the tooltip.
