...
import org.opentele.server.exception.ClinicianException
...
class ClinicianService {
    ...
    def delete(Clinician clinicianInstance) {
        def patientGroups = Clinician2PatientGroup.findByClinician(clinicianInstance)*.patientGroup
    ...
    def ClinicianCommand getClinicianCommandForEdit(long id) {
        ...
        if (clinicianInstance) {
            ...
            command.groupIds = Clinician2PatientGroup.findAllByClinician(clinicianInstance).collect { it.patientGroupId }
            return command
        }
        return null
    }
