...
class ClinicianServiceSpec extends IntegrationSpec {
    ...
    def "test get current logged in clinician"() {
        setup:
        clinicianService.springSecurityService = Mock(SpringSecurityService)
        clinicianService.springSecurityService.currentUser >> User.findByUsername("HelleAndersen")
    ...
    def "test get current logged in clinician when not a clinician"() {
        setup:
        clinicianService.springSecurityService = Mock(SpringSecurityService)
        clinicianService.springSecurityService.currentUser >> User.findByUsername('NancyAnn') // User er patient

        expect:
        !clinicianService.currentClinician
    }

    @Ignore // Delete is currently not working
    def "test delete"() {
        setup:
        def command =buildClinicianCommand(roleIds: [Role.list().first().id], groupIds: PatientGroup.list()[0..1]*.id)
        def clinician = clinicianService.createClinician(command)
